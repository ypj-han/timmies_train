...
def export_val_predictions():
    print("This is val!!!!!!!!!!!!!!!!!!!!")
    model = YOLO(r"./runs/refine_trainadded/weights/best.pt")

    # 预测验证集（val）上的所有图像并保存结果
    results = model.predict(
        source=r"./datasets_added/valid/images",  # val 图像路径
        conf=0.25,                # 置信度阈值
        save=True,                # 保存预测图像
...
def export_test_predictions():
    ...
    # 预测验证集（val）上的所有图像并保存结果
    results = model.predict(
        source=r"./datasets_added/test/images",  # val 图像路径
        conf=0.25,                # 置信度阈值
        save=True,                # 保存预测图像
        save_txt=True,            # 保存预测框标签文件（YOLO格式）
        save_conf=True,           # 保存每个框的置信度
...
def test_yolo():
    ...
    model = YOLO(r"./runs/refine_trainadded/weights/best.pt")
    # 在测试集上评估模型
    metrics = model.val(data="./datasets_added/data.yaml", split="test")
